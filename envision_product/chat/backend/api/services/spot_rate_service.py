...
from ..models import SpotRateRequest, CarrierSpotCost, CostDetail, SpotRateMatrixResponse
...
# Path will be services_dir/../../data/SPOT_RATE_V1.csv which resolves correctly
...
class SpotRateService:
    ...
    def _filter_by_lane(self, filtered_df: pd.DataFrame, request: SpotRateRequest) -> pd.DataFrame:
        """
        Applies lane filters to the DataFrame based on request parameters.
        
        Args:
            filtered_df (pd.DataFrame): DataFrame to filter.
            request (SpotRateRequest): Request containing filter parameters.
    ...
    async def query_spot_rate_matrix(self, request: SpotRateRequest) -> SpotRateMatrixResponse:
        """
        Queries spot rate data and generates a 7-day rate matrix for each carrier.

        Args:
            request (SpotRateRequest): The request model containing query parameters.

        Returns:
            SpotRateMatrixResponse: The response model containing carrier cost details.
        """
...
if __name__ == "__main__":
    ...
    async def run_test_query():
        """Test function to verify the service works correctly."""
        service = SpotRateService()
        
        # Test query - using data from the CSV sample
        test_request = SpotRateRequest(
        ...
        try:
            response = await service.query_spot_rate_matrix(test_request)
            print("Test Query Results:")
            print(f"Carriers found: {len(response.spot_costs)}")
            print(f"Date range: {response.shipment_date}")
            for carrier in response.spot_costs:
                print(f"Carrier {carrier.carrier}:")
                for detail in carrier.cost_details:
                    print(f"  {detail.ship_date}: {detail.total_spot_cost}")
        except Exception as e:
